  ...
  switch (event) {
    case ESP_GATTC_OPEN_EVT: {
      ...
      if (param->open.status != ESP_GATT_OK) {
        ESP_LOGW(TAG, "[%s] OPEN_EVT failed, status (%d), app_id (%d)", to_str(this->address_).c_str(),
    ...
    case ESP_GATTC_CFG_MTU_EVT: {
      ...
      if (param->cfg_mtu.status != ESP_GATT_OK) {
        ESP_LOGW(TAG, "[%s] CFG_MTU_EVT failed, status (%d)", to_str(this->address_).c_str(),
          param->cfg_mtu.status);
        report_error();
        break;
      }
      ESP_LOGV(TAG, "[%s] CFG_MTU_EVT, MTU (%d)", to_str(this->address_).c_str(), param->cfg_mtu.mtu);
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
      ...
      if (uuid == this->service_uuid_) {
        ESP_LOGD(TAG, "[%s] SEARCH_RES_EVT service (%s) found", to_str(this->address_).c_str(),
          this->service_uuid_.to_str().c_str());
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT: {
      ...
      {
        ESP_LOGE(TAG, "[%s] SEARCH_CMPL_EVT service (%s) not found", to_str(this->address_).c_str(),
          this->service_uuid_.to_str().c_str());
        report_error();
        break;
      }
        ...
        if (this->char_uuid_ == esp32_ble_tracker::ESPBTUUID::from_uuid(result.uuid)) {
          ...
          if (auto status = esp_ble_gattc_read_char(ble_host_->gattc_if, this->conn_id_, 
            ...
            ESP_LOGW(TAG, "[%s] read_char error sending read request, status (%d)",
              to_str(this->address_).c_str(), status);
...
std::string MyHomeIOT_BLEClient::to_str(uint64_t address) const {
  char buffer[20];
